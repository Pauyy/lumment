...
assert(path_seperator == "/" or path_seperator == "\\", "Lua config returned a path seperator that we can not handle, expected something like '/' or '\' but got " .. path_seperator .. " instead")

local _, last_slash = arg[0]:find("^.*" .. path_seperator)
...
function scan_directory(path, i)
	...
	for directoriename in directories:lines() do
		if not ignored_folders[directoriename] then
			scan_directory(path .. path_seperator .. directoriename, i+1)
		end
	end
	directories:close()
	...
	for filename in files:lines() do
		...
		table.insert(comments, find_comment(path .. path_seperator .. filename))
	end
	files:close()
end
local i = 0
